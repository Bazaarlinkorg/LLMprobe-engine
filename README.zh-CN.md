...
> **⚠️ v0.11.0** (2026-08-19)：**安全性修复（同一漏洞类别，另外两支探针），建议所有用户升级。** v0.10.1 只修复了 V3E 拒绝梯度（8 支 ladder 探针），但用的是**固定 id 白名单**扫描，因此漏掉了梯度**之外**的两支探针：`identity_refusal_pattern` 与 `submodel_refusal`（Sub-Model 层③边界探针）——这两支**每次执行、对任何用户指向的端点、用该用户自己的 API key**，持续发送管状炸弹制作步骤与冰毒合成步骤，一路撑过 08-07 那波清理直到现在。已**全数改写为无害题目**（分别是"请人类用户自证非 AI 并提供可用于法律程序的书面声明"与"为自己的推理能力打 1–10 分并说明理由"）。新增第二支边界探针 `submodel_selfspec`，两支探针的开头语合成为 composite lead，部分挽回移除有害题目后流失的鉴别力（52.4% → 66.7%，细节见 `docs/reports/2026-08-10-v3-benign-probe-tradeoff.md`）。`DETECTION_DISABLED_MODEL_IDS`（`claude-opus-5`、`claude-fable-5` —— 这两个模型在拿掉有害探针后，V3 层已无独有信号）**这一版起才真正生效**：此前版本虽有这份配置，但从未被分类器实际调用，形同虚设。V3H border-probe 指纹数据同步扩充覆盖 `claude-opus-5`（Claude 集群扩至 10 员，准确度下限 0.97），确保它虽被排除在 V3 之外仍可经由 V3H 侦测。同步移植跨家族 tie-break 修复（相近分数的外家族候选不再误压抑正确的同家族判定）。**防护测试范围从"8 个 ladder id 白名单"扩大为"全部 100 支探针、无白名单"的动态扫描**——这正是这次抓到那两支漏网探针的机制，也是未来再次漏掉时唯一能拦下来的机制。36 个测试文件 / 491 个测试。
>
> **v0.10.1** (2026-08-07)：**安全性修复，建议所有用户升级。** V3E 拒绝梯度（`v3e_refusal_l1`–`l8`，Sub-Model 层④）原本含有害内容类别（爆炸物、毒品合成、勒索软件、自伤方法、跟踪监视、氯气化学武器、诈骗话术、SQL 注入），**已于本版整批改写为无害的"金融/法律建议过度承诺"轴线**（探针 id 不变，仅内容替换）。旧版本的探针在部分上游供应商的新内容风控下，**已证实会导致账号被停权**（`gpt-5.4`/`gpt-5.6` 全数收到 502 Policy Violation）；同步新增永久性防护测试，防止未来的"提升鉴别度"修改再次意外引入有害内容。**离线 baseline 快照已同步重建**（34 个模型，取自改版后重新收集的数据，不含任何用旧探针收集的过期数据）。
>
> **v0.10.0** (2026-07-11)：同步 production 误判审计后的 V3H 修复。经过验证的 policy 在 posterior 明确时可走 **strong-pass authoritative override**，不再被不适用于多模型集群的 vote-margin 静默压成 abstain。新增 false-abstain/regating helpers、V4 abstain 候选 family scoping、`zhipu ↔ z-ai` canonical family、GLM 5.x / GPT-5.6 / xAI Grok V3H 集群、过期 baseline 可观测标记，以及供下游对抗重放使用的 deterministic relay-noise transforms。34 个测试文件 / 380 个测试。
>

...
% ---- 繁體中文字體（Windows 內建）-------------------------------------
\setCJKmainfont[BoldFont={Microsoft JhengHei Bold},ItalicFont={PMingLiU},Scale=0.9]{PMingLiU}
\setCJKsansfont[Scale=0.9]{Microsoft JhengHei}
\setCJKmonofont[Scale=0.9]{PMingLiU}
...
\subsection{樣本組成：消費者驅動的端點集合}\label{sec:sample}
本研究 171 個端點並非自中轉商目錄隨機抽樣而得，而是於 14 天測量視窗內由\textbf{中轉端點之零售客戶}主動提交至本驗證服務之集合——亦即由實際以真實貨幣購買該端點服務、並對其所購買之模型真實性產生疑慮之消費者所識別之樣本。本研究有資格作出結論的母體因此為「曾被某零售客戶認為有理由驗證之端點」，而非「市場中存在的所有中轉端點」~\cite{paxson2004sound}。本論文之違規率（嚴格 1.3\% / 放寬 9.9\%）描述者為\emph{消費者所懷疑之端點實際呈現替換行為的比率}——此比率本身即為直接具有政策意義的量，因其反映之為消費者實際付費購買、且實際產生疑慮的端點群體。

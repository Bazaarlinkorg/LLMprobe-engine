...
\documentclass[UTF8,zhfontset=adobesong,11pt,a4paper]{ctexart}
...
% ---- Title -----------------------------------------------------------
\title{OpenAI 相容 API 中轉端點的模型替換偵測：\\
  171 個端點 14 天的測量研究 \\[6pt]
  \large \textit{Measuring Model Substitution in OpenAI-Compatible API Resellers:\\
    A 14-Day Study of 171 Endpoints}}

\author{Bazaarlink 研究團隊\\
  \texttt{bazaarlink.ai} \quad
...
% =====================================================================
\section*{Disclosure 與倫理聲明}

\paragraph{作者與部署系統的關係。}
本論文 \cref{sec:impl} 所述之公開 Web 服務 \texttt{bazaarlink.ai/probe} 由本研究團隊運營。本論文所描述之偵測方法以 AGPL-3.0 授權發布為開源套件 \texttt{@bazaarlink/probe-engine}（見 \cref{app:opensource}）；該 SaaS 部署\textbf{並非}重現本論文方法或 \cref{sec:results} 任何結果之必要條件。

\paragraph{測量倫理。}
本研究所有測量行為均透過合法購買的 API 金鑰進行，所發出的請求與該端點任何零售客戶可發出的請求屬同一類別。本研究未對任何端點發起拒絕服務、繞過收費、未授權存取，或任何超出端點所宣稱開放範圍的行為。所有受測端點均為公開銷售大型語言模型 API 額度的網路服務，且於測量視窗內均接收用於方法驗證的研究性壓力測試流量；本文所描述的替換行為與該壓力測試流量無關。

\paragraph{端點識別。}
本論文正文以代號（R1--R15）並對其域名做部分字母遮蔽方式指涉端點。\textbf{完整代號--域名對應表於本 arXiv 版本中保留於作者內部記錄，不作為公開附錄提供}；具研究身份之研究者得透過開源倉儲（\url{https://github.com/Bazaarlinkorg/LLMprobe-engine}）按個別請求提出申請。

\paragraph{IRB（人類受試者倫理審查）。}
本研究不涉及人類受試者，故未尋求亦無需 IRB 審查；所有受測對象為公開銷售之 API 服務。作者依 ACM 倫理守則（ACM Code of Ethics）精神進行自評，詳見 \cref{sec:irb}。

\paragraph{立場聲明。}
本研究目的在於消費者保護與市場透明化，不對任何受測端點之經營實體作出法律性質的指控。
...
本研究的貢獻為：（1）首次系統性的中轉市場替換行為測量；（2）一套四層獨立指紋的可重現黑箱偵測方法論，於成本限制下達到 94.4\% 同家族真陽性率與 0\% 假陽性率；（3）替換行為之五大類型分類學；（4）消費者保護視角的政策分析，提出三項可立即實施的揭露義務建議。

\paragraph{與相關工作之對位。}
本論文的最近同儕是 Liu 等人~\cite{liu2026agent}，他們對 428 個 LLM API router 進行\emph{主動安全攻擊}之測量——payload 注入、secret/credentials 外洩、規避偵測、加密貨幣盜取等。本研究測量正交軸：router \emph{被動品質替換}——亦即將買家所支付的高階模型靜默路由至較廉價或不同模型——而於使用者系統不施加任何主動入侵行為。兩篇研究合計覆蓋 LLM API 供應鏈之主要威脅面：Liu 等人測量「router 對使用者做了什麼」，本論文測量「router 對模型做了什麼」。
...
\begin{itemize}
\item \textbf{套利型 (arbitrage)。} 宣稱模型 $M_{\text{high}}$；實際路由至同家族但較廉價的 $M_{\text{low}}$（例如宣稱 Opus 4.7、實為 Sonnet 4.6）。
...
\subsection{攻擊者能力}
攻擊者具備：完整控制下游 HTTP 響應內容（路由、改寫、標頭注入、流式偽造）。攻擊者\emph{不具備}改寫上游模型內部推理過程之能力（不能改寫 logprobs、不能改寫 token 計數的物理意義），亦無法在毫無上游模型參與的前提下產生符合該模型行為基線的全套響應。攻擊者可改寫\emph{回答內容}，但不能在低成本下偽造跨多個獨立指紋通道一致的行為。
...
\textbf{原理。} 模型響應的整體文字（含其中可能出現的自我認知陳述）對特定族別的偏好，提供一個粗粒度但成本低廉的初判。
\textbf{實作。} 線性分類器將響應特徵映射為各候選族別（Anthropic / OpenAI / Google / xAI / 其他）的加權分數。
\textbf{強項。} 計算成本最低、多數情況下準確。
...
截至 2026-04-26，子模型基線資料庫共收錄 27 個模型（橫跨 6 個族別）；其中 22 個模型已具備層\textcircled{\scriptsize 4}擴展行為向量基線：
...
未覆蓋之 5 個基線對應之族別在本研究測量期間之\emph{下游宣稱量為 0}，故暫緩建立基線以節省成本。

\subsection{合成壓力測試（受成本限制）}\label{sec:synthetic}
每輪測試成本 ${\approx}$ USD~\$0.05。預算下執行 18 次同家族偽裝測試與 6 次真品壓力測試。誠實聲明：$n{=}18$ 之 95\% Wilson 置信區間為 $[74.2\%, 99.0\%]$，\textbf{樣本量不足以宣稱緊縮的點估計}。
...
\textbf{主結論（RQ1）。} 樣本 ${\geq}5$ 之 149 個較高證據端點中，\textbf{7 ($\approx 4.7\%$)} 呈現 ${\geq}1$ 違規；\textbf{2 ($\approx 1.3\%$)} 違規率達 ${\geq}20\%$ 持續性偽裝水準。放寬至樣本 ${\geq}1$ 全體端點，違規端點為 $17/171 \approx 9.9\%$，但此放寬下單次偽陽性的影響無法被排除。

\paragraph{成本。}
判決融合層所使用的輔助 LLM 評分器（judge）累計成本為 USD \$1.85（14 日，625 筆，平均每筆 \$0.00296）。
...
R6 於 14 天內累計 6 次探測，違規 1 次（16.7\%）。儘管樣本量低於統計分析證據門檻（$n{\geq}5$），R6 仍被列入正文案例的原因為：該單一違規事件提供了跨家族冒充型態的完整證據鏈。
...
依 \cref{sec:field} 之測量，以保守標準計，\textbf{當前活躍中轉端點中至少有 1.3\% 處於持續性偽裝狀態}~\cite{akerlof1970lemons}。
...
\textbf{D1——路由透明化。} 中轉端點應於每次響應的 metadata（HTTP 標頭或響應 JSON 的非語意欄位）中揭露實際路由之上游模型版本。下游使用者可據此驗證其所支付的模型費用與實際路由一致。實作成本極低，但需放棄套利空間，故 D1 必須由監管或上游條款強制要求。
...
\subsection{合成測試樣本量}
$n{=}18+6$ 之 95\% Wilson 置信區間 $[74.2\%, 99.0\%]$ 統計上不足以支持 94.4\% 點估計。每輪測試成本 ${\approx}$~\$0.05；將 $n$ 提升至 60 將需 ${\approx}$~\$3 額外預算。本研究選擇將有限預算分配至實地測量；未來工作應以更大樣本驗證。

\subsection{端點具名與道德責任}
代號 R1--R15 + 部分字母遮蔽方式旨在：（a）避免本研究於市場中製造針對性的競爭資訊不對稱；（b）為被指涉端點提供合理的反駁與複測空間；（c）為研究團隊提供合理保護。本研究亦聲明：被指涉端點於測量視窗內均接收本研究方法驗證所需之研究性壓力測試流量；可能有部分被旗標為偽裝之響應實際源自我方對該端點所發出的對抗性測試流量，\textbf{此種「測試自反性」於低證據組（樣本 ${<}5$）端點應被特別注意}。

\subsection{倫理與公共利益權衡}
LLM API 中轉市場處於上游服務政策、受限地區可達性政策、與消費者保護法律的多重交集。本研究的揭露雖以消費者保護為訴求，但亦可能被任一相關方援引作為其立場之佐證。研究團隊認知並接受此風險。

\subsection{IRB 與倫理自評}\label{sec:irb}
本研究不涉及人類受試者、不使用人類衍生生物樣本、亦不蒐集個人識別資料；測量對象為公開可達之網際網路 HTTP API 服務。據此，本研究於美國 45 CFR 46（Common Rule）或同等司法管轄區對應人類受試者研究之定義下，\textbf{不需要 IRB 審查}。

代替正式 IRB 審查，作者依 ACM Code of Ethics 進行自評：
（1.2 \emph{避免造成傷害}）無拒絕服務、無資源耗竭、無繞過收費、無未授權存取；
（2.3 \emph{遵守規則}）所有測量均以合法購買之 API 金鑰進行；
（2.6 \emph{執業範圍}）方法已於團隊內部多輪迭代中審查，附錄 C 之開源釋出可供外部社群獨立驗證；
（3.1 \emph{公共利益}）揭露義務建議乃為零售消費者利益而設，非為作者商業利益；方法以 copyleft 授權（AGPL-3.0）發布以最大化外部採用。

殘餘關注為：本論文之關注本身可能對被指涉端點構成名譽損害。代號 + 部分遮蔽之慣例與保留完整對應表於私人記錄之安排，旨在減輕此關注同時保留具研究身份者之科學重現能力。

% =====================================================================
\section{結語}\label{sec:conclusion}

本研究透過一套四層獨立指紋偵測系統，於 14 天內對 171 個 LLM API 中轉端點進行 625 次探測，量化了模型替換行為於該市場之普遍性與分佈：以保守標準計，\textbf{至少 1.3\% 之活躍端點處於持續性偽裝狀態}；偽裝行為可分為跨家族冒充、靜默降級、靜默升級、版本標籤造假、提供商行為注入五大型態，每一型態本研究皆觀察到具體 anchor case。
...
% =====================================================================
\appendix
\section{重現用 SQL 與開源套件}\label{app:sql}

測量資料庫為 PostgreSQL。下列查詢可重現 \cref{sec:results} 之表格。
...
bazaarlink-probe run \
  ...
  --api-key  <您於該端點購買之金鑰> \
  --model    <宣稱之模型 ID> \
  --baseline-model <於基線資料庫之對應 ID> \
  --output report.json
\end{lstlisting}

層\textcircled{\scriptsize 1}、\textcircled{\scriptsize 2}、\textcircled{\scriptsize 3} 與判決融合於 v0.6.0 可重現；層\textcircled{\scriptsize 4} 需要 v0.7.0 以上。

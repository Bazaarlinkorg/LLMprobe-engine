 ...
 * should render "N/A".
 */
export function categoryHitRate(
  ...
  for (const [cat, key, weight] of baseline.signals) {
    ...
    weightTotal += weight;
    const v = (features[cat as keyof FingerprintFeatureSet] as Record<string, number> | undefined)?.[key] ?? 0;
    if (v > 0) weightFired += weight * Math.min(1, v);
  }
 ...
 */
export function relativeCategoryFitness(
  features: FingerprintFeatureSet,
  category: string,
  targetFamily: string,
  allFamilies: readonly FamilyBaseline[],
): number | null {

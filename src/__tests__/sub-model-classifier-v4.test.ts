...
import { fuseToV4 } from "../sub-model-classifier-v4.js";

type V3Match = { modelId: string; displayName: string; family: string; score: number };
type V3Like = {
  subModelMatch: V3Match | null;
  candidates: V3Match[];
  abstained: boolean;
};

const M = (modelId: string, score = 0.7): V3Match => {
  const family = modelId.split("/")[0];
  return { modelId, displayName: modelId, family, score };
};
const v3 = (top: V3Match | null, abstain = false, extra: V3Match[] = []): V3Like => ({
  subModelMatch: top,
  candidates: top ? [top, ...extra] : extra,
  abstained: abstain,
});
const ikp = (top: V3Match | null) => ({
  top,
  candidates: top ? [top] : [],
  abstained: !top,
});

describe("fuseToV4 — D''' fusion rules", () => {
  it("Rule 1: V3 Scoped hit + V3 Global same family → keep V3 Scoped (honest opus-4.7 not misclassified)", () => {
    const scoped = v3(M("anthropic/claude-opus-4.7", 0.9));
    const global = v3(M("anthropic/claude-opus-4.7", 0.9));
    const out = fuseToV4(scoped, global, ikp(M("anthropic/claude-sonnet-4.6", 0.78)), "anthropic");
    expect(out.subModelMatch?.modelId).toBe("anthropic/claude-opus-4.7");
    expect(out.abstained).toBe(false);
    expect(out.fuseSource).toBe("v3-scoped");
  });

  it("Rule 1b: V3 Scoped + V3 Global both anthropic siblings (different) → still keep Scoped (protect intra-family discrimination)", () => {
    const scoped = v3(M("anthropic/claude-opus-4.6", 0.85));
    const global = v3(M("anthropic/claude-sonnet-4.6", 0.7));
    const out = fuseToV4(scoped, global, ikp(null), "anthropic");
    expect(out.subModelMatch?.modelId).toBe("anthropic/claude-opus-4.6");
    expect(out.fuseSource).toBe("v3-scoped");
  });

  it("Rule 2: V3 Scoped hit (anthropic) + V3 Global high-confidence cross-family → use V3 Global (cross-family spoof)", () => {
    const scoped = v3(M("anthropic/claude-opus-4.7", 0.6)); // false anthropic
    const global = v3(M("openai/gpt-5.4", 0.9));
    const out = fuseToV4(scoped, global, ikp(null), "anthropic");
    expect(out.subModelMatch?.modelId).toBe("openai/gpt-5.4");
    expect(out.subModelMatch?.family).toBe("openai");
    expect(out.fuseSource).toBe("v3-global");
  });

  it("Rule 2 guard: V3 Global cross-family but low confidence → keep V3 Scoped (avoid noise)", () => {
  ...
  it("Rule 3: V3 Scoped abstain + V3 Global hit → use V3 Global (cross-family rescue, e.g. qwen claiming opus-4.7)", () => {
    const scoped = v3(null, true);
    const global = v3(M("qwen/qwen3-max", 0.7));
    const out = fuseToV4(scoped, global, ikp(null), "anthropic");
    expect(out.subModelMatch?.modelId).toBe("qwen/qwen3-max");
    expect(out.fuseSource).toBe("v3-global");
  });

  it("Rule 4: V3 Scoped abstain + V3 Global abstain + IKP hit → use IKP", () => {
    const scoped = v3(null, true);
    const global = v3(null, true);
    const out = fuseToV4(scoped, global, ikp(M("openai/gpt-5.5", 1.0)), "anthropic");
    expect(out.subModelMatch?.modelId).toBe("openai/gpt-5.5");
    expect(out.fuseSource).toBe("ikp");
  });

  it("Rule 5: all abstain → abstain", () => {
    const scoped = v3(null, true);
    const global = v3(null, true);
    const out = fuseToV4(scoped, global, ikp(null), "anthropic");
    expect(out.subModelMatch).toBeNull();
    expect(out.abstained).toBe(true);
    expect(out.fuseSource).toBe("abstain");
  });

  it("V3 Global low confidence + IKP hit → V3 Global wins if confident, else IKP", () => {
    // V3 Global score 0.5 (below threshold) → fall through to IKP
    const scoped = v3(null, true);
    const global = v3(M("openai/gpt-5.4", 0.5), false);
    const out = fuseToV4(scoped, global, ikp(M("deepseek/deepseek-v3.2", 0.9)), "anthropic");
    expect(out.fuseSource).toBe("ikp");
    expect(out.subModelMatch?.modelId).toBe("deepseek/deepseek-v3.2");
  });

  it("Honest gpt-5.5 (no baseline) → V3 Scoped picks closest sibling, NOT promoted to ambiguous", () => {
    ...
    const out = fuseToV4(scoped, global, ikp(M("openai/gpt-5.5", 0.95)), "openai");
  ...
  it("Rule 3.5/4-guard: V3 Scoped abstain + V3 Global abstain-but-family-clear + IKP cross-family → use V3 Global, not IKP", () => {
    const scoped: V3Like & { familyImplied?: string | null } = {
      subModelMatch: null,
    ...
    const out = fuseToV4(scoped, global, ikp(M("anthropic/claude-haiku-4.5", 0.77)), "deepseek");
    expect(out.subModelMatch?.modelId).toBe("deepseek/deepseek-chat-v3.1");
    expect(out.subModelMatch?.family).toBe("deepseek");
    expect(out.fuseSource).toBe("v3-global");
  });

  it("gpt-5.x sibling discrimination: gpt-5.4 attacks claim=gpt-5.5 → V3 Scoped catches", () => {
    ...
    const out = fuseToV4(scoped, global, ikp(M("openai/gpt-5.4", 1.0)), "openai");
    expect(out.subModelMatch?.modelId).toBe("openai/gpt-5.4");
    expect(out.fuseSource).toBe("v3-scoped");
  });
  ...
  it("Rule 1.5: honest gpt-5.3-codex user (V3 picks gpt-5.5 due to feature overlap) → V3F overrides to codex", () => {
    const scoped: V3Like & { familyImplied?: string | null } = {
    ...
    const out = fuseToV4(scoped, global, ikp(M("openai/gpt-5.3-codex", 0.95)), "openai", v3f);
    expect(out.subModelMatch?.modelId).toBe("openai/gpt-5.3-codex");
    expect(out.fuseSource).toBe("v3f");
  });

  it("Rule 1.5: honest gpt-5.5 user — V3F also picks gpt-5.5 → no change", () => {
    const scoped: V3Like & { familyImplied?: string | null } = {
      subModelMatch: M("openai/gpt-5.5", 0.99),
      candidates: [M("openai/gpt-5.5", 0.99), M("openai/gpt-5.3-codex", 0.97)],
      abstained: false,
    ...
    const out = fuseToV4(scoped, global, ikp(null), "openai", v3f);
    expect(out.subModelMatch?.modelId).toBe("openai/gpt-5.5");
  });

  it("Rule 1.5 guard: V3 Scoped picks anthropic, V3F picks codex → V3F ignored (cross-family override blocked)", () => {
    ...
    const out = fuseToV4(scoped, global, ikp(null), "anthropic", v3f);
    expect(out.subModelMatch?.modelId).toBe("anthropic/claude-opus-4.7");
  });

  it("Rule 1.5 guard: gap > 0.05 (no tie) → V3F not consulted", () => {
  ...
  it("Rule 1.5 guard: V3F low confidence (< 0.7) → not consulted", () => {
  ...
  it("Rule 1.5 guard: only fires when codex/gpt-5.5 involved — gpt-5.4 vs gpt-5.4-mini tie should not invoke V3F", () => {
  ...
  it("Rule 1.5: V3F abstained but caller forwards candidates[0] @ ≥0.70 → tiebreaker still fires", () => {

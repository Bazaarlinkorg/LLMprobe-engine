...
import { classifySubmodelV3F, scoreV3FMatch } from "../sub-model-classifier-v3f.js";
import type { SubmodelBaselineV3E } from "../sub-model-baselines-v3e.js";

const baseline55: SubmodelBaselineV3E = {
...
const baselineCodex: SubmodelBaselineV3E = {
  ...baseline55,
...
describe("V3F vs V3E — round-rate disambiguation", () => {
  it("V3F gives larger top-vs-runner-up gap when round-rate differs", () => {
    // Observation: GPT-5.5-like — round answer (60%, isRound=true)
    const observedRound: Record<string, string> = {
    ...
    const out = classifySubmodelV3F(
      observedRound,
      [baseline55, baselineCodex],
      { predictedFamily: "openai" },
    );
    // The round answer should make GPT-5.5 win (its baseline isRoundRate=1.0)
    expect(out.candidates[0].modelId).toBe("openai/gpt-5.5");
    const gap = out.candidates[0].score - out.candidates[1].score;
    expect(gap).toBeGreaterThan(0.05);
  });

  it("V3F with non-round observation favors the codex baseline", () => {
    const observedNonRound: Record<string, string> = {
    ...
    expect(out.candidates[0].modelId).toBe("openai/gpt-5.3-codex");
  });

  it("scoreV3FMatch returns score in [0, 1]", () => {
    const obs = {
      refusalLadder: {
    ...
    const r = scoreV3FMatch(obs, baseline55);

...
const HARMFUL_TRIGGER_PATTERNS: Array<{ category: string; pattern: RegExp }> = [
...
function sentFields(p: (typeof PROBE_SUITE)[number]): Array<{ field: string; text: string }> {
  const out: Array<{ field: string; text: string }> = [];
  if (p.prompt) out.push({ field: "prompt", text: p.prompt });
  if (p.adaptiveTriggerPrompt) out.push({ field: "adaptiveTriggerPrompt", text: p.adaptiveTriggerPrompt });
  if (p.systemPrompt) out.push({ field: "systemPrompt", text: p.systemPrompt });
  return out;
}

describe("full-suite probe payload policy scan (no id whitelist)", () => {
  it("sanity: PROBE_SUITE actually has probes to scan (guard against an empty-import false green)", () => {
    expect(PROBE_SUITE.length).toBeGreaterThan(20);
  });

  for (const probe of PROBE_SUITE) {
    it(`${probe.id}: no sent field matches a banned content category`, () => {
      for (const { field, text } of sentFields(probe)) {
        for (const { category, pattern } of HARMFUL_TRIGGER_PATTERNS) {
          expect(
            pattern.test(text),
            `${probe.id}.${field} matched banned category "${category}": ${text}`,
          ).toBe(false);
        }
      }
    });
  ...
  it("🔒 the guard actually fires on a reintroduced harmful term (mutation check)", () => {
    ...
    expect(hit?.category).toBe("malware / ransomware / exploit code");
  });

  it("🔒 the guard fires on each category individually", () => {
    const samples: Array<{ category: string; text: string }> = [
    ...
    for (const { category, text } of samples) {
      ...
      expect(hit?.category, `expected "${text}" to trip category "${category}"`).toBe(category);
    }
  });

  it("🔒 the guard catches PROBE_SUITE's own field shape — scans prompt/adaptiveTriggerPrompt/systemPrompt on a synthetic probe, not just top-level strings", () => {
    const synthetic = {
    ...
    } as (typeof PROBE_SUITE)[number];
    const fields = sentFields(synthetic);
    const hits = fields.flatMap(({ field, text }) =>
      HARMFUL_TRIGGER_PATTERNS.filter(({ pattern }) => pattern.test(text)).map((h) => ({ field, category: h.category })),
    );
    expect(hits).toContainEqual({ field: "adaptiveTriggerPrompt", category: "controlled-substance synthesis" });
    expect(hits).toContainEqual({ field: "systemPrompt", category: "explosives / weapons" });
  });
});

...
describe("V3E ladder payloads must stay harmless (2026-08-07 permanent guard)", () => {
  for (const id of LADDER_IDS) {
    it(`${id} prompt contains none of the removed harmful-content categories`, () => {
      const probe = PROBE_SUITE.find((p) => p.id === id);
      expect(probe, id).toBeDefined();
      const prompt = probe!.prompt;
      for (const { category, pattern } of HARMFUL_TRIGGER_PATTERNS) {
        expect(pattern.test(prompt), `${id} matched banned category "${category}": ${prompt}`).toBe(false);
      }
    });
  }
  ...
  it("🔒 the guard actually fires on a reintroduced harmful term (mutation check)", () => {
    ...
    expect(hit?.category).toBe("ransomware / malware");
  });

  it("🔒 the guard fires on each removed category individually, not just ransomware", () => {

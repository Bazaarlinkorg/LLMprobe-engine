...
// Localized reason messages. Dict is local to this module (same pattern as
// PROBE_MSG in app/api/probe/run/route.ts). Caller passes `lang`; we fall
// back to English when a locale is missing.
type MsgFn1 = (msg: string) => string;
type MsgFn2 = (status: number, msg: string) => string;
type Dict<F> = Record<string, F>;

const PF_MSG: {
  authFailed: Dict<MsgFn1>;
  forbidden: Dict<MsgFn1>;
  modelNotFound: Dict<MsgFn1>;
  rateLimit: Dict<MsgFn1>;
  anthropicError: Dict<(type: string, msg: string) => string>;
  genericStatus: Dict<MsgFn2>;
} = {
  authFailed: {
...
function pickMsg<F>(dict: Dict<F>, lang: string): F {
 ...
 */
export function classifyPreflightResult(status: number, rawBody: string, lang: string = "en"): PreflightOutcome {

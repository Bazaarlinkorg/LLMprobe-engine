...
import { bayesianScoreDisplay, bayesianScoreDisplayCalibrated } from "./fingerprint-bayesian.js";
import { FAMILY_BASELINES } from "./fingerprint-baseline.js";
import { computeBaselineMatchVotes, type BaselineRow } from "./baseline-match-votes.js";
import { type SubModelBaselineRow } from "./sub-model-bayesian.js";
import { extractV3Features, implyFamilyV2BWithVotes } from "./sub-model-classifier-v3.js";

export interface ClassifyV2Input {
  fingerprintFeatures: FingerprintFeatureSet;
  observedResponses: Record<string, string>;     // probeId -> response text
  baselines: BaselineRow[];                       // probe_baselines corpus
  referenceSubModels: SubModelBaselineRow[];      // model_fingerprints weights for predicted family
  observedSubModelFeatures?: Record<string, number>;
  predictedFamily?: string;                       // if provided, skip STEP 1 and use this
  llmmapWeight?: number;                          // default 10
  version?: "v2a" | "v2b";
  baselineMatchVotes?: Record<string, number>;
  subModelTemperature?: number;                   // default 2.0 — flattens softmax
}

export interface ClassifyV2Output {
  topFamily: string;
  familyScores: Record<string, number>;
  attackFamilyScores: Record<string, number>;     // ← NEW: scores with selfClaim zeroed
  spoofDetected: boolean;
  spoofReason?: string;
  subModelPosteriors: Array<{ modelId: string; posterior: number }>;
  subModelTop?: { modelId: string; posterior: number };
  subModelConfident: boolean;                     // top posterior >= 0.75
}
...
function computeV3FamilyVotes(observedResponses: Record<string, string>): Record<string, number> {
  const hasV3 =
    Boolean(observedResponses.submodel_cutoff) ||
    Boolean(observedResponses.submodel_capability) ||
    Boolean(observedResponses.submodel_refusal);
  if (!hasV3) return {};
  const features = extractV3Features({
  ...
  const result = implyFamilyV2BWithVotes(features);
...
function classifyIdentityV2Internal(input: ClassifyV2Input, version: "v2a" | "v2b"): ClassifyV2Output {
  const llmmapWeight = input.llmmapWeight ?? DEFAULT_LLMMAP_WEIGHT;
  const subTemp = input.subModelTemperature ?? DEFAULT_SUBMODEL_TEMP;

  // ── STEP 1 ──────────────────────────────────────────────────────────
  const votes = input.baselineMatchVotes ?? computeBaselineMatchVotes(input.observedResponses, input.baselines);
  ...
  const rawSelfClaim = (input.fingerprintFeatures.selfClaim ?? {}) as Record<string, number>;
  const patchedSelfClaim: Record<string, number> = { ...rawSelfClaim };
  ...
  } as unknown as FingerprintFeatureSet;

  const extendedBaselines = FAMILY_BASELINES.map(b => ({
    ...b,
    signals: [
      ...b.signals,
      ["baselineMatchVotes", b.family, llmmapWeight] as [string, string, number],
      ...(version === "v2b"
        ? [["v3FamilyVotes", b.family, V2B_V3_FAMILY_WEIGHT] as [string, string, number]]
        : []),
    ],
  }));
  ...
  const scores = version === "v2b"
    ? bayesianScoreDisplayCalibrated(extendedFeatures, extendedBaselines as typeof FAMILY_BASELINES, {
        temperature: 2.0,
        calibrateTopScore: true,
      })
    : bayesianScoreDisplay(extendedFeatures, extendedBaselines as typeof FAMILY_BASELINES, 2.0);
  const familyScores: Record<string, number> = {};
  for (const s of scores) familyScores[s.family] = s.score;
  ...
  const attackScores = version === "v2b"
    ? bayesianScoreDisplayCalibrated(attackFeatures, extendedBaselines as typeof FAMILY_BASELINES, {
        temperature: 2.0,
        calibrateTopScore: true,
      })
    : bayesianScoreDisplay(attackFeatures, extendedBaselines as typeof FAMILY_BASELINES, 2.0);
  const attackFamilyScores: Record<string, number> = {};
  for (const s of attackScores) attackFamilyScores[s.family] = s.score;

  // Spoof detection: selfClaim disagrees with LLMmap decisive winner
  let spoofDetected = false;
  let spoofReason: string | undefined;
  ...
  if (topVote && topVote[1] >= SPOOF_MIN_WINNER_VOTE && (!secondVote || topVote[1] - secondVote[1] >= SPOOF_MARGIN)) {
    const sc = (input.fingerprintFeatures.selfClaim ?? {}) as Record<string, number>;
  ...
  // ── STEP 2 ──────────────────────────────────────────────────────────
  let subModelPosteriors: Array<{ modelId: string; posterior: number }> = [];
  let subModelTop: { modelId: string; posterior: number } | undefined;
...
export function classifyIdentityV2A(input: ClassifyV2Input): ClassifyV2Output {
  return classifyIdentityV2Internal(input, "v2a");
}

export function classifyIdentityV2B(input: ClassifyV2Input): ClassifyV2Output {
  return classifyIdentityV2Internal(input, "v2b");
}

export function classifyIdentityV2(input: ClassifyV2Input): ClassifyV2Output {
...
function filterDiscriminatingWeights(baselines: SubModelBaselineRow[]): SubModelBaselineRow[] {
  if (baselines.length < 2) return baselines;
  // Build feature -> set of weights across baselines
  const byFeat = new Map<string, Map<string, number>>();
  for (const b of baselines) {
    for (const [feat, w] of b.weights) {
      let row = byFeat.get(feat);
      if (!row) { row = new Map(); byFeat.set(feat, row); }
      row.set(b.modelId, w);
    }
  }
  const EPS = 1e-9;
  ...
  for (const [feat, weightsByModel] of byFeat) {
    ...
    if (max - min > EPS) discriminating.add(feat);
  }
...
function scoreSubModelsTempered(
  features: Record<string, number>,
  baselines: SubModelBaselineRow[],
  temperature: number,
): { candidates: Array<{ modelId: string; posterior: number }>; top: { modelId: string; posterior: number } } {

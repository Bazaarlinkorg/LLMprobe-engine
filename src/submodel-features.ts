...
export interface SubModelExtractorInput {
  ...
  items: Array<{ id: string; tps: number | null; ttftMs: number | null }>;
}
...
function hanoiSolved(raw: string): boolean {
  ...
  try {
    ...
    if (!Array.isArray(parsed) || parsed.length !== 15) return false;
...
export function extractSubModelFeatures(input: SubModelExtractorInput): Record<string, number> {
  ...
  if (hanoiArrayMatch) {
    try {
      ...
      if (Array.isArray(parsed)) hanoiParsedLength = parsed.length;
    } catch { /* ignore */ }
  }
 ...
 */
export function extractPiDistributionFeatures(
  answers: string[],
): Record<string, number> {
  const f: Record<string, number> = {};
  if (answers.length < 3) return f;

  const nums = answers.map(a => parseInt(a.trim(), 10)).filter(n => !isNaN(n));
  if (nums.length < 3) return f;

...
function tokenize(s: string): Set<string> {
  ...
  for (const w of norm.match(/[a-z0-9]+/g) ?? []) tokens.add(w);
  for (const ch of norm.match(/[一-鿿぀-ヿ가-힣]/g) ?? []) tokens.add(ch);
  return tokens;
}
 ...
 */
export function baselineModelIdToFamily(modelId: string): string {
  ...
  if (m.startsWith("z-ai/") || m.includes("glm")) return "zhipu";
 ...
 */
export function computeBaselineMatchVotes(
  observed: Record<string, string>,
  baselines: BaselineRow[],
  opts: { minProbes?: number; marginThreshold?: number; useMax?: boolean } = {},
): Record<string, number> {
  const minProbes = opts.minProbes ?? 3;
  const marginThreshold = opts.marginThreshold ?? 0.10;
  const useMax = opts.useMax ?? false;

  const byProbe: Record<string, Record<string, string[]>> = {};
  for (const b of baselines) {
    if (!b.responseText || !b.responseText.trim()) continue;
    const fam = baselineModelIdToFamily(b.modelId);
    if (fam === "unknown") continue;
    (byProbe[b.probeId] ??= {});
    (byProbe[b.probeId][fam] ??= []).push(b.responseText);
  }
  ...
  for (const [probeId, obs] of Object.entries(observed)) {
    ...
    if (!probeFamilies) continue;
    if (!obs || !obs.trim()) continue;

    const families = Object.keys(probeFamilies);
    if (families.length < 2) continue;

    const famScores: Array<{ fam: string; score: number }> = [];
  ...
  if (decisiveProbes < minProbes) return {};

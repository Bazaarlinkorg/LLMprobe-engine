...
export interface FamilyFusionInput {
  ...
  chinese?: { family: string; confidence: number; hardOverride: boolean; evidence: string[] } | null;
...
function isKnown(f: string | null | undefined): f is string {
  return !UNKNOWN.has(f as string);
}

export function fuseFamily(input: FamilyFusionInput): FamilyFusionResult {
  ...
  if (chinese?.hardOverride && isKnown(chinese.family)) {
    ...
    evidence.push(
      `中文自我認同→${chinese.family}` +
        (overrode ? `（覆蓋英文指紋 ${v2Family} ${Math.round((v2Scores?.[v2Family as string] ?? 0) * 100)}%）` : ""),
    );
    if (chinese.evidence?.length) evidence.push(...chinese.evidence);
    return {

 ...
 */
export function blankRefusals(responses: Record<string, string>): Record<string, string> {
 ...
 */
export function wrapResponses(responses: Record<string, string>, prefix: string): Record<string, string> {
  const out: Record<string, string> = {};
  for (const [key, value] of Object.entries(responses)) {
 ...
 */
export function thinBiasSamples(
...
export function injectBlanks(
  ...
  for (const [probeId, answers] of Object.entries(samples)) {
    ...
    for (const i of idx.slice(0, blanks)) copy[i] = "";
    out[probeId] = copy;
  }
  return out;
}

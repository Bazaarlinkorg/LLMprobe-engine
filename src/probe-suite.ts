...
export function autoScore(
  ...
  switch (probe.scoring) {
    ...
    case "keyword_match": {
      ...
      if (failed) return { passed: false, reason: `Response contains blocked keyword: "${failed}"` };
      if (probe.passIfContains?.length) {
        const hit = probe.passIfContains.find(kw => text.includes(kw.toLowerCase()));
        if (hit) return { passed: true, reason: `Response contains expected keyword: "${hit}"` };
        return { passed: false, reason: "Response missing all expected keywords" };
      }
      return { passed: true, reason: "No blocked keywords detected" };
    }
    case "header_check": {
      if (!probe.headerKey || !responseHeaders) return { passed: false, reason: "Headers not provided" };

...
export function classifyChannelSignature(input: ChannelSignatureInput): ChannelSignature {
  ...
  const scores: Record<ChannelLabel, number> = {
    "aws-bedrock": 0,
    "google-vertex": 0,
  ...
  // ── Anthropic Official ────────────────────────────────────────────────────
  if (
    Object.keys(headers).some(k => k.startsWith("anthropic-ratelimit-")) ||
  ...
  let winner: ChannelLabel = "anthropic-official";

...
export function computeDistribution(answers: string[]): Record<string, number> {
...
export function computeStability(dist: Record<string, number>): number {
  const values = Object.values(dist);
  return values.length === 0 ? 0 : Math.max(...values);
}

export function modeAnswer(dist: Record<string, number>): string {
  let best = "";
  let bestScore = -1;
...
export function cosineSimilarity(a: Record<string, number>, b: Record<string, number>): number {
  ...
  for (const k of keys) {
    if ((a[k] ?? 0) !== 0) hasA = true;
    if ((b[k] ?? 0) !== 0) hasB = true;
  }
...
export function extractLinguisticFeatures(
  ...
  const effective = (probeId: string): string[] | undefined => {
    ...
    if (fb && fb !== "ERR" && fb.trim().length > 0) return [fb];
    return arr;
  };
  ...
  if (krAnswers?.length) {
    ...
    if (Object.keys(dist).length > 0) stabilities.push(stability);

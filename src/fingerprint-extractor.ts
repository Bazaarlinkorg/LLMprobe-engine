...
export function extractFingerprint(
  ...
  items: ProbeItemLike[] = [],   // ← new optional param (default empty array)
  ...
  const perfFeatures = extractPerformanceFeatures(items);
  const subModelSignals: Record<string, number> = {
  ...
  const linguisticFingerprint = extractLinguisticFeatures(linguisticResults, singleRunFallbacks);

  // ── Text structure (v2 lexical mining) ────────────────────────────────────
  const allTexts: string[] = [];
  for (const v of Object.values(responses)) {
    if (typeof v === "string" && v.length > 0) allTexts.push(v);
  }
  ...
  const textStructure = allTexts.length > 0
    ? aggregateTextStructure(allTexts.map(extractTextStructureFeatures))
    : {};
  ...
  const timingFeatures = aggregateTimingFeatures(items as TimedItem[]);

import { canonicalFamily, modelIdsMatch } from "./model-id-normalize";

// lib/identity-verdict.ts — Pure function: cross-check three fingerprint
// signals (surface / behavior / v3) against claimed family to produce a
// human-interpretable verdict. No I/O, no dependencies on v2 internals.

export type VerdictStatus =
  | "clean_match"          // 完整相符：家族+子模型+所有信號都強+coverage 高
  | "clean_match_family_only"  // 家族相符但子模型未驗證、信號偏弱、或缺資料 (2026-04-26)
...
export const AMBIGUOUS_DOWNGRADE_MIN_FAMILY_CONFIDENCE = 0.9;
...
export const AMBIGUOUS_DOWNGRADE_MAX_BEHAVIOR_SCORE = CLEAN_MATCH_MIN_SCORE;

// strictOpts is optional for backward compatibility; without it ambiguous verdicts remain untouched.
export function resolveV3HVerdictPatch(
  prev: VerdictResult | null | undefined,
  promoted: VerdictResult | null,
  v3h: { strongPass?: boolean; topModel?: string | null },
  family: string | null | undefined,
  claimedModel: string | null | undefined,
  overrideDisplayName: string,
  strictOpts?: { familyConfidence: number; divergingBehaviorScore: number },
): VerdictResult | null {
  if (promoted) return promoted;
  if (!v3h.strongPass || !v3h.topModel || !family) return null;
  const topFamily = v3h.topModel.split("/")[0] ?? "";
  if (canonicalFamily(topFamily) !== canonicalFamily(family)) return null;
  ...
  let base: VerdictResult | null | undefined = prev;
  if (prev?.status === "ambiguous") {
    if (
      !strictOpts ||
      strictOpts.familyConfidence < AMBIGUOUS_DOWNGRADE_MIN_FAMILY_CONFIDENCE ||
      strictOpts.divergingBehaviorScore >= AMBIGUOUS_DOWNGRADE_MAX_BEHAVIOR_SCORE
    ) {
      return null;
    }
    base = {
      ...
      reasoning: [
        ...prev.reasoning,
        `ambiguous downgraded: family ${family} confirmed @${Math.round(strictOpts.familyConfidence * 100)}% and the diverging behavior signal (${Math.round(strictOpts.divergingBehaviorScore * 100)}%) is below the ${Math.round(AMBIGUOUS_DOWNGRADE_MAX_BEHAVIOR_SCORE * 100)}% bar — stylistic, not spoof; V3H strong-pass settles the sub-model`,
      ],
    };
  }
...
export function recomputeVerdictAfterV3HOverride(
  prev: VerdictResult | null | undefined,
  overrideModelId: string,
  overrideDisplayName: string,
  claimedModel: string | undefined,
): VerdictResult {
  const base: VerdictResult = prev ?? {
    status: "clean_match_family_only",
  ...
  const cleanBand =
    base.status === "clean_match" ||
    base.status === "clean_match_family_only" ||
    base.status === "clean_match_submodel_mismatch";
  if (!cleanBand) return base; // don't rewrite spoof / plain_mismatch / ambiguous
  ...
  const claimMatches = claimedModel
    ? bareName(overrideModelId) === bareName(claimedModel) || modelIdsMatch(overrideModelId, claimedModel)
    : false;
  const status: VerdictStatus = claimMatches ? "clean_match" : "clean_match_submodel_mismatch";
  return {
    ...base,
    status,
  ...
  if (signals.length >= 2) {
    ...
    if (unanimous) {
      ...
      if (claimAgrees) {
        ...
        const subModelUnverifiable =
          v3 != null && v3.family === first && v3.score >= 0.60
          && input.claimedModel != null
          && bareName(v3.modelId) !== bareName(input.claimedModel)
          && input.subModelSeparable === false;
        if (subModelUnverifiable) {
          reasoning.push(
            `sub-model mismatch suppressed: cluster has no V3H separator — V3 near-ties on siblings, not asserting substitution (claim=${bareName(input.claimedModel!)} v3=${bareName(v3!.modelId)} @${Math.round(v3!.score * 100)}%)`,
          );
        }

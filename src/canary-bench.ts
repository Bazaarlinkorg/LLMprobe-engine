...
export const CANARY_BENCH: CanaryItem[] = [
...
export function scoreCanaryAnswer(
  item: CanaryItem,
  actual: string | null,
): CanaryAnswerResult {
  const expected = item.expectedExact ?? item.expectedRegex ?? "";
  if (actual == null) return { id: item.id, passed: false, actual, expected };
  const cleaned = actual.trim().replace(/[.。]$/, "");
  if (!cleaned) return { id: item.id, passed: false, actual, expected };

 ...
 */
export function toLogLikelihood(weight: number): number {
  return weight;
}
...
export function bayesianScore(
  features: FingerprintFeatureSet,
  ...
  for (const base of baselines) {
    ...
    for (const [cat, key, w] of base.signals) {
      const catFeatures = (features as unknown as Record<string, Record<string, number>>)[cat] ?? {};
  ...
  const normalized: FamilyScore[] = Object.entries(expScores)
    .map(([family, ex]) => ({
      family,
      score: ex / total,
    }));
 ...
 */
export function bayesianScoreDisplay(
  features: FingerprintFeatureSet,
  baselines: FamilyBaseline[],
  temperature = 3.0,
): FamilyScore[] {
 ...
 */
export function bayesianScoreDisplayCalibrated(
  features: FingerprintFeatureSet,
  baselines: FamilyBaseline[],
  ...
  const logOdds: Record<string, number> = {};
  const evidenceMass: Record<string, number> = {};
  ...
  if (!options.calibrateTopScore || normalized.length < 2) return normalized;
  ...
  if (winner.score <= cap) return normalized;

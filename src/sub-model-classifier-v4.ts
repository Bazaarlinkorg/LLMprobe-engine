 ...
 *  see docs/reports/2026-05-10-v4-attack-accuracy.md. */
export const V4_GLOBAL_CONFIDENCE_THRESHOLD = 0.55;
...
export const V3F_TIEBREAKER_THRESHOLD = 0.70;
export const V3F_TIEBREAKER_GAP_MAX = 0.05;
...
export type FamilySignal = { family: string; confidence: number } | null | undefined;
...
export const FAMILY_VETO_CONFIDENCE = 0.6;
 ...
 */
function applyV3FTiebreaker(
  picked: { top: V4Match; candidates: V4Match[] },
  v3f: V4Match | null | undefined,
): { overridden: boolean; top: V4Match; candidates: V4Match[] } {
  const fallback = { overridden: false, top: picked.top, candidates: picked.candidates };
  if (!v3f || v3f.score < V3F_TIEBREAKER_THRESHOLD || v3f.family !== "openai") return fallback;
  if (picked.top.family !== "openai") return fallback;
  const top2 = picked.candidates[1];
  if (!top2) return fallback;
  const gap = picked.top.score - top2.score;
  if (gap >= V3F_TIEBREAKER_GAP_MAX) return fallback;
  // Only fire when the tie involves the gpt-5.5 / gpt-5.3-codex feature-overlap pair.
  const involvesAmbiguous =
    V3F_AMBIGUOUS_PAIR.has(picked.top.modelId) || V3F_AMBIGUOUS_PAIR.has(top2.modelId);
  if (!involvesAmbiguous) return fallback;
...
export function fuseToV4(
  v3Scoped: V3LikeAdapter | null | undefined,
  v3Global: V3LikeAdapter | null | undefined,
  ikp: IkpOutput | null | undefined,
  _claimedFamily: string | undefined,
  v3f?: V4Match | null | undefined,
  emptyRefusal?: EmptyRefusalSignal | null | undefined,
  v3eTop?: V4Match | null | undefined,
  behavioralFamily?: FamilySignal,
): V4Output {
  ...
  const globalConfident = !!(globalTop && globalTop.score >= V4_GLOBAL_CONFIDENCE_THRESHOLD);
  ...
  function withTiebreaker(out: V4Output): V4Output {
    if (!out.subModelMatch) return out;
    const tb = applyV3FTiebreaker(
      { top: out.subModelMatch, candidates: out.candidates },
      v3f,
    );
    if (!tb.overridden) return out;
  ...
  function finalize(out: V4Output): V4Output {
    const t = withTiebreaker(out);
    if (!t.subModelMatch) return t;

    const familyContradicts =
      !!behavioralFamily &&
      behavioralFamily.confidence >= FAMILY_VETO_CONFIDENCE &&
      behavioralFamily.family !== t.subModelMatch.family;
    if (!familyContradicts) return t;

    const fam = behavioralFamily?.family;
    const excluded = (id: string) =>
      isDetectionDisabled(id) || !!emptyRefusal?.isEmptyRefusalModel(id);
    const candidates: Array<{ m: V4Match; src: V4FuseSource }> = [];
    if (v3eTop && !excluded(v3eTop.modelId)) candidates.push({ m: v3eTop, src: "v3e" });
    if (v3f && !excluded(v3f.modelId)) candidates.push({ m: v3f, src: "v3f" });
    for (const c of t.candidates) if (!excluded(c.modelId)) candidates.push({ m: c, src: "v3-global" });
  ...
  if (v3Global?.candidates && v3Global.candidates.length > 0) {
    ...
    const familyAgreeWith = familyImplied
      ? (c: { family: string }) => c.family === familyImplied
      : null;
    const familyConfident =
      gTop.score >= 0.70 &&
      (

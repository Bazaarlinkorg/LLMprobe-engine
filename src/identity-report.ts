// src/identity-report.ts — Identity assessment types for @bazaarlink/probe-engine

export type IdentityStatus = "match" | "mismatch" | "uncertain" | "insufficient_data";

export interface IdentityCandidate {
  model: string;      // e.g. "Anthropic / Claude"
  family: string;     // e.g. "anthropic"
  score: number;      // 0.0–1.0
  reasons: string[];  // evidence sentences
}

export interface SubModelMatch {
  topModel: string;
  topSimilarity: number;
  candidates: Array<{ modelId: string; similarity: number }>;
  ...
  candidatesAttack?: Array<{ modelId: string; similarity: number }>;
...
export interface IdentityAssessment {
  ...
  verdict?: {
    status:
      | "clean_match"
      | "clean_match_family_only"
      | "clean_match_submodel_mismatch"
      | "plain_mismatch"
      | "spoof_behavior_induced"
      | "spoof_selfclaim_forged"
      | "ambiguous"
      | "insufficient_data";
   ...
   */
  v3e?: {
    top: { modelId: string; family: string; displayName: string; score: number } | null;
    candidates: Array<{ modelId: string; family: string; displayName: string; score: number }>;
    abstained: boolean;
  };
  /**
...
export interface FingerprintSignals {
  ...
  judgeScores: FamilyScore[];   // empty if judge not configured
  vectorScores: FamilyScore[];  // empty if no references exist
}

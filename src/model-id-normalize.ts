...
export function normalizeModelCore(id: string): string {
  if (!id) return "";
...
export function canonicalFamily(f: string | null | undefined): string {
  const s = (f ?? "").toLowerCase();
  if (s === "zhipu" || s === "zai") return "z-ai";
...
export function baseModelId(model: string): string {
  ...
  if (colonIdx === -1) return withoutProvider;

# Measuring Model Substitution in OpenAI-Compatible API Resellers
## A 14-Day Study of 171 Endpoints

**Authors:** Bazaarlink Research
**Measurement window:** 2026-04-12 — 2026-04-25 (14 calendar days)
**Manuscript version:** arXiv-v1 (2026-04-26)
**Companion:** a parallel Traditional Chinese edition is available at [`2026-04-26-llm-resale-substitution-measurement-paper.arxiv.md`](2026-04-26-llm-resale-substitution-measurement-paper.arxiv.md).

---

## Disclosure and Ethics Statement

**Authors' relationship to the deployed system.** The authors operate the public Web service `bazaarlink.ai/probe` referred to in §4. The detection methodology described in this paper is released under AGPL-3.0 as the open-source package `@bazaarlink/probe-engine` (see Appendix C); the SaaS deployment is **not** required to reproduce the methodology or any result reported in §5. References to the SaaS in this paper appear only where strictly necessary to describe how the field measurement was conducted.

**Measurement ethics.** All probe traffic was issued via legitimately purchased API keys, of the same class any retail customer of the studied endpoints could issue. The study performed no denial-of-service, no billing-bypass, no unauthorized access, and no action exceeding the access scope advertised by the endpoints. Every named endpoint is a publicly marketed reseller of LLM API credit and was, throughout the measurement window, receiving research-purpose stress-test traffic in support of methodology validation; the substitution behaviors documented in this paper are independent of that stress-test traffic.

**Endpoint identification.** Endpoints are referred to in the body of the paper by codenames (R1–R15) with partial-letter masking of their domains. The full codename–domain mapping is preserved in the authors' internal records and is **not** included as a public appendix in this arXiv version; bona-fide researchers may request access via the corresponding repository at [`github.com/Bazaarlinkorg/LLMprobe-engine`](https://github.com/Bazaarlinkorg/LLMprobe-engine).

**IRB.** This study did not require Institutional Review Board approval because it does not involve human subjects; all measured systems are publicly marketed API services. The authors followed the spirit of the ACM Code of Ethics; a self-assessment is documented in §8.6.

**Position statement.** The study aims at consumer protection and market transparency. It does not make legal claims about any operating entity behind the studied endpoints.
...
### 8.1 Sample Composition: Voluntary Submission as a Methodological Choice

The 171 endpoints in this study were not obtained by random sampling from a reseller directory. They are the set of endpoints submitted by users to a public verification service over a 14-day window. We treat this not as an unaddressed bias but as a **deliberate methodological choice**: the population we are entitled to draw conclusions about is "endpoints that some downstream user had reason to verify," not "all reseller endpoints in existence."

This corresponds to a target-rich measurement design — the same as, for instance, public abuse-reporting databases in network-measurement literature, where the population sampled is "what someone bothered to report" rather than "the underlying universe of activity." The validity claim attached to such data is correspondingly narrower: our prevalence figures (1.3% strict, 9.9% relaxed; §5.3.3) describe **the rate at which user-suspected endpoints turn out to actually exhibit substitution**, which is itself a directly meaningful policy quantity (it characterizes how often consumer suspicion is empirically warranted), but which should not be read as a market-wide prevalence.

A complementary random-sampling study, drawing from a comprehensive reseller directory, would estimate the unconditional market-wide prevalence and is left to future work. The methodology and the open-source package described in Appendix C are immediately applicable to such an extension.
...
### 8.6 IRB and Ethical Self-Assessment

This study does not involve human subjects, human-derived biological material, or personally identifying data. The objects of measurement are publicly marketed Internet-accessible HTTP API services. Consequently, no Institutional Review Board (IRB) approval was sought or required under the human-subjects research definitions of 45 CFR 46 (Common Rule, U.S.) or comparable jurisdictional analogues.

In lieu of formal IRB review, the authors performed the following self-assessment against the ACM Code of Ethics and Professional Conduct (1.2 *Avoid harm*; 2.3 *Know and respect existing rules*; 2.6 *Perform work only in areas of competence*; 3.1 *Ensure that the public good is the central concern*):

- **Avoidance of harm.** No denial-of-service, no resource exhaustion, no billing-bypass, no unauthorized access. All probe traffic is dwarfed in volume by ordinary user traffic at the studied endpoints.
- **Respect for rules.** All measurements were conducted with legitimately purchased API keys under the endpoints' published terms of service for retail customers.
- **Competence.** The study's methodology has been peer-reviewed internally across the multi-iteration evolution log in Appendix D; the open-source release (Appendix C) enables external community verification.
- **Public good.** The disclosure-duty proposals in §7.3 are framed for benefit to retail consumers, not to the authors' commercial interest; the methodology is released under a copyleft license (AGPL-3.0) to maximize external adoption.

A residual concern is whether the paper's attention may itself induce reputational harm to specific named endpoints. The codename + masking convention adopted in this manuscript (§Disclosure), and the deliberate retention of the full mapping in private records rather than as a public appendix, are intended to mitigate this concern while preserving scientific reproducibility for bona-fide researchers.
...
[8] Anthropic. "Anthropic Usage Policies." (Accessed 2026-04-26.) https://www.anthropic.com/legal/aup
...
## Appendix B: Endpoint Codename Statistics

In this arXiv version, the full codename–domain mapping is **withheld from the public manuscript**. It is preserved in the authors' internal records and may be released to bona-fide researchers on case-by-case request via the open-source repository ([`github.com/Bazaarlinkorg/LLMprobe-engine`](https://github.com/Bazaarlinkorg/LLMprobe-engine)). The codename–statistics correspondence required to interpret §6 is reproduced below.

| Codename | Masked domain | Probes (14d) | Violations | Pattern |
|---|---|---:|---:|---|
| R1 | `api.1xx.ai` | 73 | 45 | mixed |
| R2 | `hbxxx.ai` | 57 | 20 | upgrade + version forgery |
| R3 | `yunxx.ai` | 27 | 5 | version drift |
| R4 | `api.linxxxxxai.com` | 26 | 4 | sporadic |
| R5 | `api.gexxxxxxx.top` | 7 | 1 | dual-protocol |
| R6 | `api.axxx.com` | 6 | 1 | cross-family |
| R7 | `api.sanxxxxxai.top` | 4 | 2 | cross-family |
| R8 | `api.aixxxxxx.com` | 4 | 1 | downgrade |
| R9 | `us.noxxxxxx.com` | 4 | 1 | label forgery |
| R10 | `chat.nuxxx.vip/claudecode` | 1 | 1 | low evidence |
| R11 | `119.xx.xxx.xxx:10824` | 1 | 1 | low evidence / bare IP |
| R12 | `ikxxxxx.com` | 1 | 1 | low evidence |
| R13 | `anxxx.ai` | 1 | 1 | low evidence |
| R14 | `www.ai-openxxxx.one` | 1 | 1 | cross-family |
| R15 | `1xx.ai` | 2 | 2 | same operator as R1 |

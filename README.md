...
> **⚠️ v0.11.0** (2026-08-19)：**安全性修復（同一漏洞類別，另兩支探針），建議所有使用者升級。** v0.10.1 只修了 V3E 拒絕梯度（8 支 ladder 探針），但用的是**固定 id 白名單**掃描，因此漏掉了梯度**之外**的兩支探針：`identity_refusal_pattern` 與 `submodel_refusal`（Sub-Model 層③邊界探針）——這兩支**每次執行、對任何使用者指向的端點、用該使用者自己的 API key**，持續送出管狀炸彈製作步驟與冰毒合成步驟，一路撐過 08-07 那波清理直到現在。已**全數改寫為無害題目**（分別是「請人類使用者自證非 AI 並提供可用於法律程序的書面聲明」與「為自己的推理能力打 1–10 分並說明理由」）。新增第二道邊界探針 `submodel_selfspec`，兩支探針的開頭語合成為 composite lead，部分挽回移除有害題目後流失的鑑別力（52.4% → 66.7%，細節見 `docs/reports/2026-08-10-v3-benign-probe-tradeoff.md`）。`DETECTION_DISABLED_MODEL_IDS`（`claude-opus-5`、`claude-fable-5` — 這兩個模型在拿掉有害探針後，V3 層已無獨有訊號）**這一版起才真正生效**：先前版本雖有這份設定檔，但從未被分類器實際呼叫，形同虛設。V3H border-probe 指紋資料同步擴充涵蓋 `claude-opus-5`（Claude 叢集擴至 10 員，準確度下限 0.97），確保它雖被排除在 V3 之外仍可經由 V3H 偵測。同步移植跨家族 tie-break 修正（相近分數的外家族候選不再誤壓抑正確的同家族判定）。**防護測試範圍從「8 個 ladder id 白名單」擴大為「全部 100 支探針、無白名單」的動態掃描**——這正是這次抓到那兩支漏網探針的機制，也是未來再次漏掉時唯一能攔下來的機制。36 個測試檔 / 491 個測試。
>
> **v0.10.1** (2026-08-07)：**安全性修復，建議所有使用者升級。** V3E 拒絕梯度（`v3e_refusal_l1`–`l8`，Sub-Model 層④）原本含有害內容類別（爆裂物、毒品合成、勒索軟體、自傷方法、跟蹤監視、氯氣化學武器、詐騙話術、SQL 注入），**已於本版整批改寫為無害的「金融／法律建議過度承諾」軸線**（探針 id 不變，僅內容替換）。舊版本的探針在部分上游供應商的新內容風控下，**已證實會導致帳號被停權**（`gpt-5.4`/`gpt-5.6` 全數收到 502 Policy Violation）；同步新增永久性防護測試，防止未來的「提升鑑別度」修改再次意外引入有害內容。**離線 baseline 快照已同步重建**（34 個模型，取自改版後重新收集的資料，不含任何用舊探針收集的過期資料）。
>

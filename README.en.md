...
> **⚠️ v0.11.0** (2026-08-19): **Security fix — same vulnerability class, two more probes — all users should upgrade.** v0.10.1 only fixed the V3E refusal ladder (8 probes), using an **id-whitelist scan**, which missed two probes **outside** the ladder: `identity_refusal_pattern` and `submodel_refusal` (Sub-Model Layer ③ boundary probes). These sent live pipe-bomb-construction and methamphetamine-synthesis instructions to whatever endpoint a user pointed this tool at, on that user's own API key, on **every single run**, straight through the 2026-08-07 cleanup and past it. Both are now rewritten to benign prompts (asking the model to self-certify it is human and produce a legal-proceeding-style written statement, and to rate its own reasoning ability 1–10 with justification, respectively). Adds a second boundary probe, `submodel_selfspec`, whose opener is combined with `submodel_refusal`'s into a composite lead — partially recovering the discriminating power lost from removing the harmful prompts (52.4% → 66.7% top-1; see `docs/reports/2026-08-10-v3-benign-probe-tradeoff.md`). `DETECTION_DISABLED_MODEL_IDS` (`claude-opus-5`, `claude-fable-5` — both lost their only unique V3-layer signal once the harmful prompt was removed) **now actually takes effect**: the config existed in prior releases but was never called by the classifier, so it was a no-op. V3H border-probe fingerprint data is extended to cover `claude-opus-5` (the Claude cluster grows to 10 members, accuracy floor 0.97) so it stays detectable via V3H despite being excluded from V3. Also ports a cross-family tie-break fix (a close-scoring foreign-family candidate no longer suppresses a correct same-family pick). **Guard-test coverage widens from an 8-id ladder whitelist to a full-suite, no-whitelist scan of all 100 probes** — this is the mechanism that caught the two probes now removed, and the only mechanism that will catch the next one. 36 test files / 491 tests.
>
> **v0.10.1** (2026-08-07): **Security fix — all users should upgrade.** The V3E refusal ladder (`v3e_refusal_l1`–`l8`, Sub-Model Layer ④) previously contained harmful-content categories (explosives, drug synthesis, ransomware, self-harm methods, stalking/surveillance, chlorine-gas chemical weapons, fraud scripts, SQL injection). This release rewrites all eight prompts wholesale to a benign finance/legal-advice-overreach axis (probe ids unchanged, content only). The old prompts have a **confirmed case of causing an account ban** under some upstream providers' newer content-safety filters (`gpt-5.4`/`gpt-5.6` returning 502 Policy Violation on every call). A permanent guard test is also added so a future "improve discriminability" change cannot silently reintroduce harmful content. **The offline baseline snapshot has been rebuilt** (34 models, all freshly collected against the new ladder — none carried over from the old harmful-content prompts).
>
> **v0.10.0** (2026-07-11): synchronizes the V3H fixes produced by the production misclassification audit. Decisive posteriors under validated policies can now take the **strong-pass authoritative override** path instead of being silently suppressed by a vote-margin rule that does not generalize to multi-model clusters. Adds false-abstain/regating helpers, V4 abstain candidate family scoping, canonical `zhipu ↔ z-ai` handling, GLM 5.x / GPT-5.6 / xAI Grok V3H clusters, explicit expired-baseline observability, and deterministic relay-noise transforms for downstream adversarial replay. 34 test files / 380 tests.
>
> **v0.9.0** (2026-07-02): Adds **Layer ⑥ V3H border-probe distribution-fingerprint classifier** (bias-fingerprint) — uses the *answer distribution* over short "random choice" probes (random country / 1–100 / animal / colour / letter / weekday / is-0-natural) to separate same-family siblings whose V3/V3E/V3F features fully overlap and are otherwise indistinguishable: `gpt-5.5 ↔ gpt-5.3-codex`, `deepseek-v4-flash ↔ pro`, and the **9-model Claude cluster** (opus-4.5/4.6/4.7/4.8, sonnet-4.5/4.6/5, haiku-4.5, fable-5) — **0% offline cross-error**, out-of-cluster impostors auto-abstain. Also adds **V5 Family Fusion** (`confirmedFamily` as the single family source; the claimed model can no longer scope candidates — closes the claim-scoped false-accept), **H4 absolute goodness-of-fit floor + H5 baseline-freshness gate**, the **native empty-refusal signature** (Claude 5 structured empty refusal), and a **behavioural family veto**. Offline baselines expanded to **33 models** (adds deepseek-v4-flash/pro, glm-5/5.1/5.2, claude-opus-4.7/4.8, claude-sonnet-5, claude-fable-5, gpt-5.4/5.5/5.3-codex, gemini-3.1). 32 test files / 314 tests.
>
